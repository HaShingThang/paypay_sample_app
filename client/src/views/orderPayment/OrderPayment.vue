      ...
      <div class="main-content">
        ...
        <div class="cake-description">
          ...
          >
            ...
            <p class="cake-price">
              ￥{{ item.unitPrice.amount.toFixed(2) }} * {{ item.quantity }} =
...
export default defineComponent({
  name: "AdminDashboard",
  computed: mapGetters("orderDetail", ["orderStatus"]),
  setup() {
    const store = useStore();
    const route = useRoute();
    onMounted(async () => {
      try {
        await store.dispatch("orderDetail/fetchOrderStatus", route.params.id);
      } catch (error) {
