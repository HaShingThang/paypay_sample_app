<template>
  <div class="cakes">
    <img :src="imagePath" :alt="$t(props.title)" class="cakes-image" />
    <p class="cakes-text">{{ $t(props.title) }}</p>
    <div class="cakes-price">
      <div class="price-text">￥{{ props.price }}</div>
      <div class="button-tick">
        <div class="price-tick" v-if="hasItemInCart(props._id)">
          <img alt="Tick" src="@/assets/images/Tick.png" />
        </div>
        <div class="price-button" v-else>
          <button class="price-submit" @click="addToCart(props)">
...
export default {
  ...
  setup(props: any) {
    ...
    const imagePath = ref(require(`@/assets/images/${props.props.image}`));
    ...
    const hasItemInCart = (id: number) => {
      return store.getters["cart/hasItemInCart"](id);
    };
    return {
      imagePath,
      addToCart,
